...
import { Upload, Download, RefreshCw, FileSpreadsheet, ChevronDown, ChevronUp, Calendar, Database, Trash2 } from "lucide-react";
...
const ALL_COLS = [
  ...
  { key: "contractAmt1",     label: "Estimated Contract Amount 1", group: "Billing" },
  ...
  { key: "contractAmt2",     label: "Estimated Contract Amount 2", group: "Billing 2" },
  ...
  { key: "revisedClassroom",    label: "Revised Classroom",         group: "Meta" },
  { key: "earlyAMRateCardKey",  label: "Early AM Rate Card Key",    group: "Meta" },
  { key: "latePMRateCardKey",   label: "Late PM Rate Card Key",     group: "Meta" },
...
export default function FC28Page() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      const finalRes = await fetch("/api/fc28/upload", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ reportDate, batchId, isFinal: true, files: [] }),
      });
      if (!finalRes.ok) { const e = await finalRes.json().catch(() => ({})); throw new Error(e.error ?? "Finalization failed"); }
      const { rowCount, fileCount } = await finalRes.json();

      setUploadLog(prev => [...prev, ``, `✅ ${rowCount?.toLocaleString()} rows · ${fileCount} files uploaded`]);
  ...
  const deleteBatch = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`/api/fc28/batches/${id}`, { method: "DELETE" });
      if (!res.ok) { const j = await res.json().catch(() => ({})); throw new Error(j.error ?? "Delete failed"); }
      if (dlBatchId === id) setDlBatchId("");
      await loadBatches();
                  ...
                  <tr key={b.id} className={`hover:bg-slate-50 transition-colors ${dlBatchId===b.id?"bg-blue-50":""}`}>
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        title="Delete batch"
                      >
                        {deletingId===b.id ? <RefreshCw className="w-3.5 h-3.5 animate-spin"/> : <Trash2 className="w-3.5 h-3.5"/>}
                      </button>
                    </td>
                  </tr>
                ))}
